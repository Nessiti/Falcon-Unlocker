...
export function ImeiServiceManager() {
  ...
  function refresh() {
    if (!initData) return;
    listAllImeiServicesAction(initData).then((result) => {
  ...
  async function handleStatus(id: string, status: ServiceStatus) {
    setBusyId(id);
    const result = await setImeiServiceStatusAction(verifiedInitData, id, status);
    setBusyId(null);
    if (result.ok) {
      refresh();
      router.refresh();
  ...
  async function handleDelete(id: string) {
    setBusyId(id);
    const result = await deleteImeiServiceAction(verifiedInitData, id);
    setBusyId(null);
    if (result.ok) {
      refresh();
      router.refresh();
  ...
  async function handleDuplicate(id: string) {
    setBusyId(id);
    const result = await duplicateImeiServiceAction(verifiedInitData, id);
    setBusyId(null);
